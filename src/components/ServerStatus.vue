<template>
  <div class="text-center">
            <span class="fa-stack text-10xl md:text-25xl mb-4">
                <i class="fa fa-stack-2x bg-status rounded-full h-inherit w-inherit"></i>
                <transition name="fade" mode="out-in">
                    <i :class="`${statusClasses} ${statusIconClasses}`" class="fa fa-fw fa-stack-1x"
                       :key="statusClasses"></i>
                </transition>
            </span>
    <p class="text-grey-darker text-2xl md:text-5xl mb-3">The servers are
      <transition name="fade" mode="out-in">
                <span :key="statusClasses" class="underline"
                      :class="statusClasses">
                    {{status}}
                </span>
      </transition>
    ...
    <transition name="fade" mode="out-in">
      <p class="text-grey-dark h4" :key="message"
         v-if="showMessage">Message: {{message}}</p>
    </transition>
    <transition name="fade" mode="out-in">
      <p class="text-grey-dark" v-if="lastChecked">
...
<script>
  ...
  export default {
    ...
    methods: {
      async getStatus () {
        try {
          let {data} = await this.$http.get('current/')
          this.setStatus(data)
        } catch (e) {
          console.log(e)
        }
